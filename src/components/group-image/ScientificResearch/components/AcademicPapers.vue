<template>
  <div class="flex" :class="['academic-papers', {
    'flex': true,
    ...
    <div v-if="$attrs.view !== 'left'" :class="direction === 'horizontal'?'w50':'w100'" style="padding-left: 20px;">
      <div class="content-two">
        ...
        <div class="content-two-box">
          <div class="sb">
            <div class="c-title">论文</div>
            <div class="text">主要发表次数分布 <span>{{ rightData.mostPublish }}</span></div>
          </div>
          <div class="pie-chart" ref="MainPie"></div>
          <div class="two-box-bottom sb">
            <div class="left">
              <div class="c-title">论文领域分布</div>
              <div class="text mt25">主要领域分布 <span>{{ rightData.mostField }}</span></div>
              <div class="docment-chart" ref="docmentPie"></div>
            </div>
            <div class="right">
              <div class="c-title">第一作者占比</div>
              <div class="column">
                <div class="text mt25">第一作者占比 <span>{{rightData.firstUserProp}}%</span></div>
              <ElProgress  class="three" type="circle" :width="110" :stroke-width="10" color="#1B528B" :percentage="rightData.firstUserProp">
                <template #default="{ percentage }">
                  <div class="percentage-value" style="color:#1B528B;">{{ percentage }}<span>%</span></div>
                </template>
              </ElProgress>
              </div>
            </div>
    ...
    <WarningSeal name="学术论文" ident='Academic_papers'/>
  </div>
...
const colorConfig2 = reactive({
  ...
  value:'',
  isUnit:true,
  icon:getImageUrl('zzsl'),
  unit:'人',
})
const leftData =  ref<{detail: Array<any>,[key: string | symbol]: any}>({
  detail:[],
})
const rightData =  ref<{detail: Array<any>,thesisField:Array<any>,[key: string | symbol]: any}>({
  detail:[],
  thesisField:[],
})
const initResult = () => {
    ...
    tooltip: {
      ...
      formatter: function(params:any) {
        ...
        if(params.length&&params.length==1){
          let unit = params[0].seriesName=="论文数量"?'篇':'人'
...
const initDocmentPie = () => {
  ...
  let data = rightData.value.thesisField
  let option = {
        color:['#FD5145','#1B528B','#00C9F2','#F7B500',],
        legend: {
          type: "scroll",
...
const dataSource = usePortraitRequest({...toRefs(props.params) as any, ident: 'Academic_papers'})
